...
async def try_execute(client, message):
    if message.content == 'connect':
        ...
        col = discord.Colour.green() if True else discord.Colour.dark_magenta()
        embed = discord.Embed(colour=col, title='New update', url=f'https://twitter.com/{server_status.user.screen_name}/status/{server_status.id})', description=server_status.text)
